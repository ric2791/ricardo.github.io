...
const concatAppKey = function (userAgent, appKey) {
  // eslint-disable-next-line prefer-template
  return appKey ? userAgent + " " + appKey : userAgent;
};
...
module.exports = function (config, mode, overrides) {
  ...
  return _.extend({}, wrap(globalValues), wrap(modeValues), {
    ...
    getErrorTemplateName() {
      return modeValues.errorTemplateName || defaults.errorTemplateName;
    },

    getErrorTemplateObject() {
      return modeValues.errorTemplateObject || defaults.errorTemplateObject;
    },
    ...
    hostScriptUrl() {
      return get(modeValues, "hostScriptUrl", "AC_HOST_SCRIPT_URL");
    },
    ...
    ngrok() {
      const creds = utils.loadJSON("credentials.json");
      if (!creds["ngrok"]) {
        console.error(`\n*** credentials.json file is missing ngrok ***\n`);
      }
      return creds["ngrok"];
    },
    ...
    userAgent(appKey) {
      return (
        modeValues.userAgent ||
        globalValues.userAgent ||
        concatAppKey(defaults.userAgent, appKey)
      );
    },

    appId() {
      return env.AC_APP_ID || modeValues.appId || globalValues.appId;
    }
  });
};
...
const defaults = {
  ...
  errorTemplate: false,
  errorTemplateName: "unauthorized",
  errorTemplateObject: {},
